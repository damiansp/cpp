...
/* Solve x^2 - 8x - 13 = 0 by iterative trial and error */
int main() {
  // curve should cross x-axis
  float p1 = -8.,    // coef for linear term
    p2 = 1.,         // coef for squared term
    const_val = -13, // const term
  ...
  for (i = 0; i < iterations; i++) {
    ...
    printf("  x appx: %f\n", middle);
    test_val = p2*pow(middle, 2) + p1*middle + const_val;
    if (test_val == 0) {
      printf("x = %f\n", middle);
      return 0;
    }
